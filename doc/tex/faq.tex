...
\subsection{Can I run SMAC on Windows?}

	We have made our best effort to ensure that SMAC works on windows. SMAC includes \texttt{.bat} files for execution that behave similarly to the unix shell scripts. Most wrappers require that you have ruby (which is available from: ruby http://rubyinstaller.org/). 
	
	Most of the included scenarios will not work but \\ \texttt{example\_saps\textbackslash scenario-win-Saps-SWGCP-sat-small-train-small-test.txt} \\
	 is a scenario that works with windows. If you are having trouble running it, try placing the example\_data directory in the example\_saps scenario (which gets lost due to the lack of symlinks).

\subsection{Can I run SMAC on Mac OS X or other Unix environments?}

	SMAC should work on any unix environment that supports Java 6. Unfortunately aside from the one Windows scenario, all the other scenarios require Linux currently.
