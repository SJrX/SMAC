...
SMAC supports two types of parameters, categorical and continuous. The former is specified as follows:
...
\vspace{5pt}
Categorical Parameters are specified as follows:
...
We have specified timeout as continuous with a default
value of 500. Any value is legally permitted so long as it's in the
interval of {[}1, 10000{]}. When drawing a random configuration out of
...
\item[Integer] Integer parameters must have all values specified as integers, even though strictly speaking the notation should permit fractional values. Additionally the default value must be a integer.
...
SMAC, HAL. ParamILS is probably the most portable. The Exactly Regex
that is used in this version is: \textbf{(Final)?\textbackslash{}s{*}{[}Rr{]}esult\textbackslash{}s+(?:(for)|(of))\textbackslash{}s+(?:(HAL)|(ParamILS)|(SMAC)|(this
wrapper))} %
.}:
